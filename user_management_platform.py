...
def create_account():
	...
	if len(username)<3 or len(password)<3:
		...
		db_users = []
		database_users = open("private_users_database.txt", "r+")
		lines = database_users.readlines()
		for line in lines:
			db_user = line.split(":")[0]
			db_users.append(db_user)
		if username not in db_users:
			toWrite = username + ":" + password + ":" + "0" + "\n"
			database_users.write(toWrite)
...
def do_login():
	...
	for line in lines:
		...
		db_user = linePieces[0]
		db_pass = linePieces[1]
		admin = linePieces[2]
		db_users.append(db_user)
...
def delete_user():
	...
	if username not in db_users:
		...
		database_users = open("private_users_database.txt", "w")
		for line in lines:
...
def change_my_password(username, current_password,privilege):
	...
	if len(password)<3:
		...
		raise Exception("Password format invalid")
	else:
		found_user = False
		db_users = []
		database_users = open("private_users_database.txt", "r")
		...
		for line in lines:
			...
			if db_user == username:
				found_user = True
				line_to_change = db_user + ":" + password + ":" + linePieces[2] + "\n"
